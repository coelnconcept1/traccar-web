...
export default () => {
  ...
  return useMemo(
    () => {
      ...
      const customIndex = availableStyles.findIndex(s => s.id === 'custom');
      if (customIndex > 0) {
        const [custom] = availableStyles.splice(customIndex, 1);
        availableStyles.unshift(custom);
      }
      return styles.filter(s => s.available).sort((a, b) =>
        a.id === 'custom' ? -1 : 1
      );
    },
    [
